...
class JsonSerializer
{
     ...
     */
    public function __construct( $closureSerializer = null)
    {
     ...
     * @throws \Core\Serialization\JsonSerializerException
     */
    ...
    public function unserializeWithTypeName($value, $typename)
    {
        $this->reset();
        $data = json_decode($value, true);
        if ($data === null && json_last_error() != JSON_ERROR_NONE) {
        ...
        return $this->unserializeDataWithTypeName($data,$typename);
    }
    /**
    ...
    protected function serializeData($value)
    {
        ...
        if (is_resource($value)) {
            throw new JsonSerializerException('Resource is not supported in Serialization');
        }
        if (is_array($value)) {
    ...
    protected function unserializeDataWithTypeName($value, $typename)
    {
        if (is_scalar($value) || $value === null) {
            return $value;
        }

     return $this->unserializeObjectWithTypeName($value, $typename);
    ...
    protected function unserializeObjectWithTypeName($value, $className)
    {
        $className = $value[static::CLASS_IDENTIFIER_KEY];
        unset($value[static::CLASS_IDENTIFIER_KEY]);
