...
namespace Core\ORM;
use \mysqli;
use \ReflectionProperty, \ReflectionObject, \ReflectionClass;


class Model extends SerializableModel

{
    ...
    private function insert ()
    {
        ...
        foreach ($array AS $key => $value)
        {
            ...
            $fieldMarkers[] = '?';
            $types[] = self::parseValueType($value);
            $values[] = &$array[$key];
        }
    ...
    {
        // build sql statement
        $sql = sprintf("DELETE FROM `%s`.`%s` WHERE `%s` = ?", self::getDatabaseName(), self::getTableName(),$field);
     ...
     * @return string
     */
    public static function parseValueType ($value)
    {
        // ints
        if (is_int($value))
            return 'i';
     ...
     * @return mixed
     */
    public static function sql($sql, $query_values = null , $return = Model::FETCH_MANY)
    {
        // shortcuts
        ...
        if($query_values != null){
           $values = $types  = array();
            ...
            foreach ($query_values as $key => $query_value){
                $types[] = self::parseValueType($query_value);
                $values[] = &$query_values[$key];
            }
            ...
            call_user_func_array(array($stmt, 'bind_param'), array_merge(array(implode($types)), $query_values));
            $stmt->execute();
    ...
    public static function PopulateWithRequest($entry, $data) {
        foreach ($data as $var => $value) {
            //Set Value
            $entry->$var = trim($value); //here you can add a filter, like htmlentities ...
        }
    ...
    {
        $count = self::sql($sql,null, Model::FETCH_ONE);
    ...
    {
        self::sql('TRUNCATE :database.:table',null, Model::FETCH_NONE);
    }
    ...
    public static function retrieveByPK ($pk)
    {
        ...
        return $reflectionObj->newInstanceArgs(array($pk, Model::LOAD_BY_PK));
    }



    /**
     * Removes an entry by primary key
     * @param $pk
     */
    public static function removeByPk ($pk)
    {
        $entry = self::retrieveByPK($pk);
        $entry->delete();
    }


    /**
     * Creates a new entry and stores it in database
     */
    ...
    /**
     * Creates an entity without storing
     * @return ReflectionClass
     */
    public static function createEntryWithoutSaving ()
    {
        $reflectionObj = new ReflectionClass(get_called_class());
    ...
    public static function hydrate ($data)
    {
        ...
        return $reflectionObj->newInstanceArgs(array($data, Model::LOAD_BY_ARRAY));
    }
     ...
     * @return mixed
     */
    public static function retrieveByField ($field, $value, $return = Model::FETCH_MANY)
    {
        ...
        $sql = sprintf("SELECT * FROM :database.:table WHERE %s %s ?", $field, $operator, $value);

        if ($return === Model::FETCH_ONE)
            $sql .= ' LIMIT 0,1';

        // fetch our records
        return self::sql($sql,[$value], $return);
    }
